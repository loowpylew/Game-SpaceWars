...
public class SpaceWars implements SWAT,Serializable
{
     ...
     * The value is also declared as static so that all instances of the class will have access to the same value.
     * So in the case a force is activated and placed into the activeStarFleet array found within SpaceWars, the deduction on the warChest
     * will apply to all instances of the class made rather than all instances having a value of 1000 bitcoin each.
     */
...
//**********************Battles************************* 
   ...
   // ***************   file write/read  *********************
    ...
    public void saveGame(String fname) throws IOException {   // uses object serialisation
        ...
        /*SpaceWars updated = new SpaceWars(admiral);

        outf.writeObject(updated);

        outf.close();*/
        for(Force temp: unitedForcesFleet){
            outf.writeObject(temp);
        }
        for(Force temp: activeStarFleet) {
            outf.writeObject(temp);
        }
       ...
       */
      public SpaceWars restoreGame(String fname) throws IOException, ClassNotFoundException {   // uses object serialisation
          try {
              ...
              Force f = null;
              f = (Force) inf.readObject();
              while (f != null) {
                  if (f.getForceState() == ForceState.DOCKED) {
                      unitedForcesFleet.add(f);
                  }
                  if (f.getForceState() == ForceState.ACTIVE || f.getForceState() == ForceState.DESTROYED) {
                      activeStarFleet.add(f);
                  }
              }
          }

          catch(EOFException e){ System.out.println("File read done");}
